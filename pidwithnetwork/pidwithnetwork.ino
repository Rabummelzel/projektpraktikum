...
int packetSize; 
EthernetUDP Udp; 
const int COMMZEIT = 5;
//.

//fixed heater/cooler values
boolean pid_control;
int heat_fix;
int cool_fix;
...
void loop() {
  ...
  //receive and handle incoming messages from the client every COMMZEIT seconds
  if ( 0 == messdauer % (1000 * COMMZEIT))
  {
  ...
  if (0 == messdauer % (1000 * REGELZEIT))   // only run regulation loop after messdauer [ms]
  {
    ...
    else analogWrite(heizer, heat_fix);
    ...
    */
    ...
    else analogWrite(cooler, cool_fix);
    ...
    String dat;
    if(pid_control)
       dat = String(regelungsZaehler) + "\t" + String(SOLL - xMittel) + "\t" + String(x) + "\t" + String(xMittel) + "\t" + String(stellwert) + "\t" + String(yCool) + "\t" + String(tag);
    else  dat = String(regelungsZaehler) + "\t" + String(SOLL - xMittel) + "\t" + String(x) + "\t" + String(xMittel) + "\t" + String(heat_fix) + "\t" + String(cool_fix) + "\t" + String(tag);
    Serial.println(dat);
...
int handleMessage(String message)
{
  ...
  }else if(message == "gpid")
  {
    sendUdp("KP_H = " + String(KP) + ", " + "TN_H = " + String(TN) + ", " + "TV_H = " + String(TV) + "\n" + "KP_C = " + String(KP_C) + ", " + "TN_C = " + String(TN_C) + ", " + "TV_C = " + String(TV_C) + "\n" + "sollwert = " + String(SOLL) + "\n"
     + "PID is turned on " + String(pid_control));
  }
  else if(message == "pidoff")
  {
    pid_control = false;
    heat_fix = 0;
    cool_fix = 0;
    sendUdp("Pid control turned off");
  }
  else if(message == "pidon")
  {
    pid_control = true;
    sendUdp("Pid control turned on");
  }else
  { 
    ...
    if(str1 == "h")                                           //set the heater values
    {
      ...
      if(str2 == "f")
      {
        if ((val > 0) || (val < 255))
        {
          pid_control = false;
          heat_fix = val;
          sendUdp("Pid control turned off and heater set to: " + String(heat_fix) + " bits");
        }else sendUdp("Value out of bounds, needs to be 0 < val < 255");
      }
    ...
    }else if(str1 == "c")                                     //set the cooler values
    {
      ...
      if(str2 == "f")
      {
        if ((val > 0) || (val < 255))
        {
          pid_control = false;
          cool_fix = val;
          sendUdp("Pid control turned off and cooler set to: " + String(cool_fix) + " bits");
        }else sendUdp("Value out of bounds, needs to be 0 < val < 255");
      }else{sendUdp("Unrecognized command"); return 0;}
